/**
 * Arthur I (V200M): men's clog — crna, bela; sizes 41–46; galleries from leon.rs.
 * npx tsx scripts/patch-arthur-i-v200m.mjs
 */
...
const PAGES = [
  {
    slug: 'arthur-i-crna',
    url: 'https://leon.rs/p/arthur-i-crna/',
    colorLabel: 'CRNA',
    name: {
      de: 'Arthur I – Schwarz',
      fr: 'Arthur I – Noir',
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Arthur-I-CRNA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
  {
    slug: 'arthur-i-bela',
    url: 'https://leon.rs/p/arthur-i-bela/',
    colorLabel: 'BELA',
    name: {
      de: 'Arthur I – Weiß',
      fr: 'Arthur I – Blanc',
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Arthur-I-BELA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
];
...
for (const page of PAGES) {
  ...
  const sizeList = [
    ...new Set([...SIZES, ...(info.sizes ?? [])].filter((s) => Number(s) >= 41 && Number(s) <= 46)),
  ...
  p.modelGroupId = 'leon-mg-arthur-i-v200m-men';
  p.name = page.name;
  p.description = {
    de: 'Herrenmodell „Arthur I“ mit bequemem, anatomischem Fussbett.',
    fr: 'Modèle homme « Arthur I » avec semelle intérieure anatomique confortable.',
    en: 'Men\'s model "Arthur I" with a comfortable anatomical footbed.',
    it: 'Modello da uomo «Arthur I» con plantare anatomico confortevole.',
  };
...
fs.writeFileSync(
  OUT_TS,
  `/* AUTO-GENERATED — arthur-i-v200m ${new Date().toISOString().slice(0, 10)} */\n` +
    `export const leonProducts = ${JSON.stringify(products, null, 2)};\n`,
  'utf8'
);
fs.writeFileSync(CACHE_PATH, JSON.stringify(cache, null, 2) + '\n', 'utf8');
console.log('Arthur I V200M done.');
