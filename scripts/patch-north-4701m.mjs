/**
 * North (4701M): men's sizes 42–49.
 * npx tsx scripts/patch-north-4701m.mjs
 */
...
const SLUGS = ['north-siva', 'north-braon', 'north-tamno-siva'];
...
for (const slug of SLUGS) {
  ...
  const priceCHF = p.variants?.find((v) => typeof v.priceCHF === 'number')?.priceCHF ?? 109;
  p.articleNumber = '4701M';
  p.category = 'men';
  p.modelGroupId = 'leon-mg-north-4701m-men';
...
fs.writeFileSync(
  OUT_TS,
  `/* AUTO-GENERATED — north-4701m sizes ${new Date().toISOString().slice(0, 10)} */\n` +
...
console.log('North 4701M fixed:', fixed);
