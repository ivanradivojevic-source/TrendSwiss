/**
 * Serene (1132): sizes 36–41 per leon.rs (no 42).
 * npx tsx scripts/patch-serene-1132.mjs
 */
...
const SERENE_SLUGS = ['serene-braon', 'serene-zlatna', 'serene-siva'];
/** leon.rs Serene 1132: 36–41 */
...
function writeLeonProducts(products) {
  fs.writeFileSync(
    OUT_TS,
    `/* AUTO-GENERATED — serene-1132 patch ${new Date().toISOString().slice(0, 10)} */\n` +
...
function rebuildSizes(p) {
  ...
  return { ...p, articleNumber: '1132', sizes, colors, variants };
}

async function verifyLeonSizes() {
  for (const slug of SERENE_SLUGS) {
    ...
    const all = [...new Set(fromVar)].sort();
    console.log('leon.rs', slug, all.join(','));
  }
}

async function main() {
  await verifyLeonSizes();
  ...
  console.log('Serene 1132 sizes →', WOMEN_SIZES.join(','), 'fixed:', fixed);
