/**
 * Thomas (V230M): men's clog — crna, bela; sizes 41–46; galleries from leon.rs.
 * npx tsx scripts/patch-thomas-v230m.mjs
 */
...
const PAGES = [
  {
    slug: 'thomas-crna',
    url: 'https://leon.rs/p/thomas-crna/',
    colorLabel: 'CRNA',
    name: {
      de: 'Thomas – Schwarz',
      fr: 'Thomas – Noir',
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Thomas-CRNA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
  {
    slug: 'thomas-bela',
    url: 'https://leon.rs/p/thomas-bela/',
    colorLabel: 'BELA',
    name: {
      de: 'Thomas – Weiß',
      fr: 'Thomas – Blanc',
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Thomas-BELA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
];
...
for (const page of PAGES) {
  ...
  p.modelGroupId = 'leon-mg-thomas-v230m-men';
  p.name = page.name;
  p.description = {
    de: 'Herrenmodell „Thomas“ mit bequemem, anatomischem Fussbett.',
    fr: 'Modèle homme « Thomas » avec semelle intérieure anatomique confortable.',
    en: 'Men\'s model "Thomas" with a comfortable anatomical footbed.',
    it: 'Modello da uomo «Thomas» con plantare anatomico confortevole.',
  };
...
fs.writeFileSync(
  OUT_TS,
  `/* AUTO-GENERATED — thomas-v230m ${new Date().toISOString().slice(0, 10)} */\n` +
    `export const leonProducts = ${JSON.stringify(products, null, 2)};\n`,
  'utf8'
);
fs.writeFileSync(CACHE_PATH, JSON.stringify(cache, null, 2) + '\n', 'utf8');
console.log('Thomas V230M done.');
