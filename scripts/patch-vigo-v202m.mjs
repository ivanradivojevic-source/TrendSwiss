/**
 * Vigo (V202M): men's clog — crna, bela; sizes 41–46; galleries from leon.rs.
 * npx tsx scripts/patch-vigo-v202m.mjs
 */
...
const PAGES = [
  {
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Vigo-CRNA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
  {
    slug: 'vigo-bela',
    url: 'https://leon.rs/p/vigo-bela/',
    colorLabel: 'BELA',
    name: {
      de: 'Vigo – Weiß',
      fr: 'Vigo – Blanc',
    ...
    galleryRe:
      /https:\/\/cdn\.leon\.rs\/wp-content\/uploads\/[^"'\\s>]+\/Vigo-BELA(?:-(\d+))?\.(?:jpg|jpeg|png|webp)/gi,
  },
];
...
for (const page of PAGES) {
  ...
  p.modelGroupId = 'leon-mg-vigo-v202m-men';
  p.name = page.name;
  p.description = {
    de: 'Herrenmodell „Vigo“ mit bequemem, anatomischem Fussbett.',
    fr: 'Modèle homme « Vigo » avec semelle intérieure anatomique confortable.',
    en: 'Men\'s model "Vigo" with a comfortable anatomical footbed.',
    it: 'Modello da uomo «Vigo» con plantare anatomico confortevole.',
  };
...
fs.writeFileSync(
  OUT_TS,
  `/* AUTO-GENERATED — vigo-v202m ${new Date().toISOString().slice(0, 10)} */\n` +
    `export const leonProducts = ${JSON.stringify(products, null, 2)};\n`,
  'utf8'
);
fs.writeFileSync(CACHE_PATH, JSON.stringify(cache, null, 2) + '\n', 'utf8');
console.log('Vigo V202M done.');
