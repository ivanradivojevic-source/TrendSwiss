/**
 * Virea (8004): sizes 36–41 per leon.rs (no 42).
 * npx tsx scripts/patch-virea-8004.mjs
 */
...
const VIREA_SLUGS = ['virea-bordo', 'virea-crna', 'virea-braon'];
/** leon.rs Virea 8004: 36–41 */
...
function writeLeonProducts(products) {
  fs.writeFileSync(
    OUT_TS,
    `/* AUTO-GENERATED — virea-8004 patch ${new Date().toISOString().slice(0, 10)} */\n` +
...
function rebuildSizes(p) {
  ...
  return { ...p, articleNumber: '8004', sizes, colors, variants };
}

async function verifyLeonSizes() {
  for (const slug of VIREA_SLUGS) {
...
async function main() {
  ...
  console.log('Virea 8004 sizes →', WOMEN_SIZES.join(','), 'fixed:', fixed);
